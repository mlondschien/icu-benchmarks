...
def main(experiment_name: str, tracking_uri: str):
    ...
    for run in runs:
        ...
        with tempfile.TemporaryDirectory() as f:
            ...
            with open(f"{f}/results.csv") as f:
                results = pl.read_csv(f)
    ...
    metrics = map(re.compile("^[a-z]+\/train\/([a-z]+)$").match, results.columns)
    ...
    for target in sources:
        ...
        for metric in metrics:
            expr = pl.coalesce(pl.when(~pl.col("sources").list.contains(t)).then(pl.col(f"{t}/train/{metric}")) for t in sources)
            ...
            model = results_n1.filter(~pl.col("sources").list.contains(target) & pl.col("alpha_idx").eq(best["alpha_idx"]) & pl.col("l1_ratio_idx").eq(best["l1_ratio_idx"]))
            cv_results.append(
                {
                **{
                    "target": target,
                    ...
                    "target_value": model.filter(~pl.col("sources").list.contains(target) & pl.col("alpha_idx").eq(best["alpha_idx"]) & pl.col("l1_ratio_idx").eq(best["l1_ratio_idx"]))[f"{target}/train/{metric}"].item()
                },
                **{f"{source}/train/": model[f"{source}/train/{metric}"].item() for source in sorted(DATASETS) if f"{source}/train/{metric}" in model.columns}}
            )
    ...
    for metric in metrics:
        ...
        with pl.Config() as cfg:
            cfg.set_tbl_cols(20)
            print(pl.DataFrame(cv_results).filter(pl.col("metric").eq(metric)).sort("target"))
