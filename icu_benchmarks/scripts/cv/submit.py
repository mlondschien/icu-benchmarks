...
def main(config: str, logs: str, hours: int, experiment_name: str, tracking_uri: str, artifact_location: str): 
    ...
    list_of_sources = [
        ...
        for dataset1 in DATASETS for dataset2 in DATASETS if dataset1 <= dataset2
    ]

    for sources, outcome in zip(list_of_sources, OUTCOMES):
        n_samples = sum(TASKS[outcome]["n_samples"][source] for source in sources)

        log_dir = Path(logs) / outcome / "_".join(sorted(dataset))
