...
@click.option("--tracking_uri", type=str, default="sqlite:////cluster/work/math/lmalte/mlflow/mlruns2.db")
    ...
    for metric in metrics:
        ...
        for i, target in enumerate(sources):
            for seed in range(20):
                filter = pl.col("target").eq(target) & pl.col("metric").eq(metric) & pl.col("seed").eq(seed)
                n_samples_grid1[:, i, seed] = get_equivalent_number_of_samples(df_nsamples.filter(filter), grid[:, i], metric)
        ...
        for i, target in enumerate(sources):
            ...
            n_samples_grid2[:, i] = np.log10(get_equivalent_number_of_samples(df_nsamples.filter(filter).group_by("n_target").agg(pl.all().median()), grid[i, :], metric))
        ...
        ax[0].set_xticks(np.arange(grid.shape[1])); ax[0].set_xticklabels([SHORT_DATASET_NAMES[x] for x in sources], rotation=45)
        ax[0].set_yticks(np.arange(grid.shape[0])); ax[0].set_yticklabels([SHORT_DATASET_NAMES[x] for x in sources])


        ax[1].set_xlabel("target dataset")
        ax[1].set_ylabel("source dataset")
        ax[1].set_xticks(np.arange(grid.shape[1])); ax[1].set_xticklabels([SHORT_DATASET_NAMES[x] for x in sources], rotation=45)
        ax[1].set_yticks(np.arange(grid.shape[0])); ax[1].set_yticklabels([SHORT_DATASET_NAMES[x] for x in sources])


        ax[2].set_xlabel("target dataset")
        ax[2].set_ylabel("source dataset")
        ax[2].set_xticks(np.arange(grid.shape[1])); ax[2].set_xticklabels([SHORT_DATASET_NAMES[x] for x in sources], rotation=45)
        ax[2].set_yticks(np.arange(grid.shape[0])); ax[2].set_yticklabels([SHORT_DATASET_NAMES[x] for x in sources])

        ax[1].set_title(f"{get_method_name(experiment_name)} ({METRIC_NAMES[metric]})")
        log_fig(fig, f"1v1/{metric}.png", client=client, run_id=run.info.run_id, bbox_inches="tight", pad_inches=0.1)
